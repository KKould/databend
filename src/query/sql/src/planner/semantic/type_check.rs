...
impl<'a> TypeChecker<'a> {
        ...
        let box (scalar, data_type): Box<(ScalarExpr, DataType)> = match expr {
            ...
            Expr::Cast {
                ...
                let target_type = if data_type.remove_nullable() == DataType::Variant {
                    ...
                    //     if let ColumnEntry::VirtualColumn(virtual_column) = guard.column(column.column.index).clone() {
                    //         if let ColumnEntry::BaseTableColumn(source_column) = guard.column(virtual_column.source_column_index).clone() {
                    //             if let Some(table_ty) = <&DataType as Into<Option<TableDataType>>>::into(&target_type) {
    ...
    fn try_rewrite_variant_function(
        ...
        let result = self.resolve(&Expr::ColumnRef {
            span,
            column: ColumnRef {
                database: None,
                table: None,
        ...
        let target_type = match func_name {
            "get_by_keypath" | "get" => DataType::Nullable(Box::new(DataType::Variant)),
            "get_by_keypath_string" | "get_string" => DataType::Nullable(Box::new(DataType::String)),
            _ => unreachable!(),
        };
        if target_type != result.as_ref().1 {
            ...
            return Ok(Some(Box::new((
                ScalarExpr::CastExpr(CastExpr {
                    ...
                    target_type: Box::new(DataType::Null),
                }),
                target_type,
            ))));
        }
