...
/// Matches: Limit -> [EvalScalar ->] Filter -> Scan
///
...
impl RulePushDownLimitFilterScan {
    pub fn new() -> Self {
        Self {
            ...
            matchers: vec![
                // Limit -> Filter -> Scan
                match_op!(Limit -> Filter -> Scan),
                // Limit -> EvalScalar -> Filter -> Scan
                match_op!(Limit -> EvalScalar -> Filter -> Scan),
            ],
        }
    }
}
...
impl Rule for RulePushDownLimitFilterScan {
    ...
    fn apply(&self, s_expr: &SExpr, state: &mut TransformResult) -> Result<()> {
        ...
        let new_filter_expr = filter_expr.replace_children(vec![Arc::new(new_scan_expr)]);
