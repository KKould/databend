...
use crate::servers::http::v1::query::string_block::{BlocksSerializer, StringBlock};
        ...
        let (data, next_uri) = if is_final {
            ...
            match state.state {
                ExecuteStateKind::Running | ExecuteStateKind::Starting => match r.data {
                    None => (Arc::new(BlocksSerializer::empty()), Some(make_state_uri(&id))),
                    Some(d) => {
                ...
                ExecuteStateKind::Failed => (Arc::new(BlocksSerializer::empty()), Some(make_final_uri(&id))),
                ExecuteStateKind::Succeeded => match r.data {
                    None => (Arc::new(BlocksSerializer::empty()), Some(make_final_uri(&id))),
                    Some(d) => {
                        let uri = match d.next_page_no {
                            Some(n) => Some(make_page_uri(&id, n)),
