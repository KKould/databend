...
impl FilterBuilder for BloomBuilder {
    ...
    fn add_key<K: Hash>(&mut self, key: &K) {
        ...
        key.hash(&mut hasher);
        self.builder.set(&hasher.finish())
    }
    ...
    fn build(&mut self) -> std::result::Result<Self::Filter, Self::Error> {
        Ok(BloomFilter {
            filter: mem::replace(
                &mut self.builder,
                Bloom::new(self.bitmap_size, self.items_count)
...
impl BloomBuilder {
    pub fn create(bitmap_size: usize, items_count: usize) -> Result<Self> {
        Ok(Self {
            bitmap_size,
            items_count,
            builder: Bloom::new(bitmap_size, items_count)?,
        })
    }
}
        ...
        Ok((
            Self {
                filter: Bloom::from_slice(buf)
                    .map_err(|e| BloomCodecError::new(format!("fail to decode bloom: {}", e)))?,
            },
            buf.len(),
        ))
    }
