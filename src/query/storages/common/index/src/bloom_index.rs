...
impl BloomIndex {
    ...
    pub fn calculate_ngram_nullable_column_digest<'a>(
        ...
        let (value, _) = eval_function(
            ...
            [
                (arg, data_type.clone()),
                (
                    Value::Scalar(Scalar::Number(NumberScalar::UInt32(gram_size as u32))),
        ...
        Ok((0..value.len()).filter_map(move |i| {
    ...
    pub fn calculate_ngram_nullable_column<'a>(
        func_ctx: &'a FunctionContext,
        arg: Value<AnyType>,
        data_type: &'a DataType,
        gram_size: usize,
    ) -> Result<impl Iterator<Item = StringColumn> + 'a> {
        let arg_len = arg.len();
        let (value, _) = eval_function(
            None,
            "ngram",
        ...
        Ok((0..value.len()).into_iter().filter_map(move |i| {
            value.index(i).and_then(|scalar| {
                if let ScalarRef::Array(column) = scalar {
                    return Some(StringType::try_downcast_column(&column).unwrap());
                }
                None
            })
        }))
    ...
    pub fn ngram_hash(s: &str) -> u64 {
        cityhash64_with_seeds(s.as_bytes(), 0, 217728422)
    }
    ...
    fn find(
        ...
        let contains = if self.version == V2BloomBlock::VERSION {
            ...
            let w = !words.iter().any(|word| {
                scalar_map
                    .get(&Scalar::String(word.to_string()))
                    .is_some_and(|digest| {
                        println!("K: {}", !filter.contains_digest(*digest));
...
struct ColumnFilterBuilder {
    ...
    gram_size: usize,
    bitmap_size: usize,
    builder: Option<FilterImplBuilder>,
}
...
impl BloomIndexBuilder {
    pub fn add_block(&mut self, block: &DataBlock) -> Result<()> {
        ...
        for (index, index_column) in self.bloom_columns.iter_mut().enumerate() {
            ...
            if index_column.is_ngram {
                for digests in BloomIndex::calculate_ngram_nullable_column_digest(
                    &self.func_ctx,
                    Value::Column(column),
                    &data_type,
                    ...
                    if index_column.builder.is_none() {
                        index_column.builder = Some(FilterImplBuilder::Bloom(BloomBuilder::create(
                            index_column.bitmap_size,
                            block.num_rows() * digests.len(),
                        )?))
                    }
                    index_column
                        .builder
                ...
                let builder = index_column
                    .builder
                    .get_or_insert_with(|| FilterImplBuilder::Xor(Xor8Builder::create()));
                ...
                if validity.as_ref().map(|v| v.null_count()).unwrap_or(0) > 0 {
                    ...
                    builder.add_digests(it);
                } else {
                    builder.add_digests(column.deref());
                }
            }
        }
    ...
    pub fn finalize(&mut self) -> Result<Option<BloomIndex>> {
        ...
        for column in self.bloom_columns.iter_mut() {
            let Some(builder) = &mut column.builder else {
                continue;
            };
